...
%% API
-export([start/0, check/1, check/2]).
    ...
    case block_opening(Word, IgnoreBlocks) of
        ...
        {open, CloseBlock} ->
            case find_close_pattern(RestLine, CloseBlock) of
                {closed, RestLine2} ->
                    find_block(RestLine2, IgnoreBlocks, CleanLine);
                no_closed ->
                    {in_block, CloseBlock, CleanLine}
            end
    end.

-spec block_opening(iodata(), [sheldon_config:ignore_block()]) ->
...
block_opening(Word, [#{open := OpenBlock, close := CloseBlock} | Rest]) ->
    case re:run(Word, OpenBlock) of
        nomatch ->
            block_opening(Word, Rest);
        _ ->
            {open, CloseBlock}
    end.

-spec find_close_pattern([iodata()], sheldon_config:regex()) ->
...
find_close_pattern([Word | RestWords] = _Line, CloseBlock) ->
    case re:run(Word, CloseBlock) of
        nomatch ->
            find_close_pattern(RestWords, CloseBlock);
        _ ->
            {closed, RestWords}
    end.
...
check_words([], Misspelled, _Config) ->
    {ok, Misspelled};
...
correctly_spelled(Word, Config = #{lang := Lang}) ->
    case sheldon_utils:is_number(Word) orelse ignore(Word, Config) of
        false ->
            sheldon_dictionary:member(Word, Lang);
        true ->
            true
    end.

-spec ignore(string(), sheldon_config:config()) -> boolean().
ignore(Word, _Config = #{ignore_words := IgnoredWords, ignore_patterns := Patterns}) ->
    lists:member(
