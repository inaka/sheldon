...
iodata_check(_Config) ->
    ...
    ok = sheldon:check(["Hi, I am (testing the spelling) checker"]),
    ok = sheldon:check([$H, [<<"i">>, " I am"], <<" (testing the spelling) checker !">>]),
    #{misspelled_words := [#{line_number := 1, word := "thedfs"}]} =
        sheldon:check(["Hi, I am \'tes", <<"ting\' the">>, $d, $f, "s.,; spelling checker"]),
    ok = sheldon:check(<<"Hi, I am (testing the spelling) checker">>),
    ok.
...
ignore_words(_Config) ->
    ...
    ok = sheldon:check(<<"testing again. Sheldon should skip ttttthis">>, Config2),
    #{misspelled_words := [#{line_number := 1, word := "vvvvv"}]} =
        sheldon:check(<<"testing again. Sheldon should skip ttttthis",
                        " but sheldon doesn't skip vvvvv">>,
                      Config2),
    ok.
...
multiline(_Config) ->
    {ok, Bin} = file:read_file([code:priv_dir(sheldon), "/../test/files/multiline.txt"]),
    #{bazinga := _,
      misspelled_words :=
        ...
        sheldon:check(Bin),
    #{bazinga := _, misspelled_words := [#{line_number := 1, word := "multiline"}]} =
        sheldon:check(Bin, #{ignore_words => ["Sheldon", "superwrong", "fsdfdsd"]}),
    ok =
        sheldon:check(Bin, #{ignore_words => ["Sheldon", "superwrong", "fsdfdsd", "multiline"]}),
    ok.

-spec ignore_blocks(config()) -> ok.
ignore_blocks(_Config) ->
    {ok, BlockInline} =
        file:read_file([code:priv_dir(sheldon), "/../test/files/block_inline.txt"]),
    #{bazinga := _,
      misspelled_words :=
    ...
    #{bazinga := _,
      misspelled_words :=
          [#{line_number := 2, word := "mistakke"}, #{line_number := 7, word := "miiisstake"}]} =
        sheldon:check(BlockInline,
                      #{ignore_blocks => [#{open => "^open_block$", close => "^close_block$"}]}),
    {ok, BlockMultiLine} =
        file:read_file([code:priv_dir(sheldon), "/../test/files/block_multiline.txt"]),
    #{bazinga := _,
      misspelled_words :=
          [#{line_number := 2, word := "mistakke"},
           #{line_number := 4, word := "open_block"},
    ...
    {ok, BlockMultiLine2} =
        file:read_file([code:priv_dir(sheldon), "/../test/files/block_multiline_no_end.md"]),
    #{bazinga := _,
      misspelled_words :=
          [#{line_number := 2, word := "mistakke"},
           #{line_number := 4, word := "open_block"},
    ...
    #{bazinga := _, misspelled_words := [#{line_number := 2, word := "mistakke"}]} =
        sheldon:check(BlockMultiLine2,
                      #{ignore_blocks => [#{open => "^open_block$", close => "^close_block$"}]}),
    ok.
